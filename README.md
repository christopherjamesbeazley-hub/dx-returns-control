...
- CSV upload for refreshing the active in-memory dashboard dataset and adding an in-session snapshot point.
- Upload confirmation text showing the last processed filename, row count, and captured snapshot number.
- Erase-all control for clearing the active dataset, generated outputs, upload snapshot history, filters, ROI assumptions, and email/LLM status.
...
Uploaded CSV files are loaded only in the browser session. They do not overwrite `src/data/returns.csv`. Each successful upload replaces the active dataset and adds one in-session snapshot point, so a second upload can be compared with the first. The Erase all button clears the active dataset, generated outputs, upload snapshot history, filters, ROI assumptions, and email/LLM status; refreshing the page reloads the committed synthetic demo CSV.

The file picker is cleared after every upload attempt, so a revised CSV with the same filename can be uploaded again and still trigger recalculation.
...
- With uploaded CSVs, snapshot comparison uses only the successful uploads from the current browser session; after Erase all, a new upload sequence starts cleanly.
